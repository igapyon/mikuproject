...
現行CLIの主な名詞空間は `ai`、`state`、`import`、`export`、`report` である。新仕様候補の `inspect`、`validate`、`diff`、`apply`、`convert`、`export` とは異なる分類である。
...
| generic `inspect` | `ai detect-kind`、`state summarize` はある | 部分対応。JSON種別・概要に限定され、外部artifact全般の能力・損失・diagnosticsを返さない | 新コマンド候補。read-onlyな構造化resultをG3で定義 |
...
| semantic `diff` | `state diff --before workbook.json --after workbook.json` | 部分対応。二つのworkbook JSONが前提で、現行import規則によるsummaryである | C1のhuman gateに必要な意味上のdiffとして契約・fixture化 |
| Patchの適用 | `state apply-patch --state ... --in ...` | 実装済み。次のworkbook JSONを出力する | 再利用候補。ただしhuman gate、pre/post validate、atomicityを新契約で定義 |
...
| stable structured result | JSON本文と `--diagnostics json` はある | 部分対応。全コマンド共通のresult envelopeやnext action契約はない | `result` / `diagnostics` schemaをG3で統一 |
| stable diagnostics / exit | diagnostics version、0/1/2のexit code、明示error codeを一部持つ | 部分対応。messageからcodeを推定する経路が残り、location・loss・normalization・retryabilityが一様でない | Agentが文言解析せず分岐できる契約へ再設計 |
...
| safe output | `--out`で指定した既存ファイルを無確認で上書きし、`writeFileSync`で直接書く | 未対応。atomic writeや既定のoverwrite拒否はない | `--force`等の明示許可、atomic write、部分成果物非公開を必須化 |
...
| atomicity | 直接`writeFileSync`する | 失敗時にpartial artifactを公開しない契約が必要 |
...
| rewrite候補 | command dispatchとI/O、result/diagnostics contract、安全出力、semantic diff、test runner | 新仕様の副作用分類、Agent分岐、atomicity、共通conformanceを満たさない |
